    ...
    const stats = [
        {
            ...
            subtitle: "Till Today",
            icon: (
                <svg className="w-12 h-12 p-2 rounded-full bg-white text-black" fill="currentColor" viewBox="0 0 24 24">
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-transparent">
            <div className="max-w-4xl w-full">
                <h1 className="text-3xl font-bold text-center mb-8 text-black">Patient Dashboard</h1>

                {/* Flex container with background for blue cards */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {stats.map((stat, index) => (
                            ...
                            >
                                ...
                                {/* Card content */}
                                <div>
                                    <h3 className="text-lg font-semibold text-black">{stat.title}</h3>
                                    <p className="text-3xl font-bold mt-1 text-black">{stat.value}</p>
                                    <p className="text-sm opacity-90 text-black">{stat.subtitle}</p>
                                </div>
                            </div>
                        ))}
                    </div>
